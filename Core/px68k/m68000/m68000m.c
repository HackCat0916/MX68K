...
void c68k_init(void)
{
    ...
    C68k_Set_WriteW(&C68K, Memory_WriteW);
        ...
        C68k_Set_Fetch(&C68K, 0xfe0000, 0xffffff, (uintptr_t)IPL);
}
...
int32_t m68000_execute(int32_t cycles)
{
	...
	char buf[128];
	snprintf(buf, sizeof(buf), "[m68000m] CPU_Emu=%d cycles=%d", Config.CPU_Emu, cycles);
	mx68k_log(buf);
	switch(Config.CPU_Emu)
	{
	case 0:// c68k Emu
	{
		int32_t ret = C68k_Exec(&C68K, cycles);
		snprintf(buf, sizeof(buf), "[m68000m] C68k_Exec returned %d", ret);
		mx68k_log(buf);
		return ret;
	}
	case 1:// Musashi Emu
