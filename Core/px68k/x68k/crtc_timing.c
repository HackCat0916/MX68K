...
// ---------------------------------------------------------------------------------------
//
// MX68K port note (P641): verbatim from MPX68K (YosAwed/MPX68K, merge
// 0d096dab8048e64f0e850f35c500e9e746929347) apart from two mechanical
// adaptations, both documented in Patches/Core/p641_crtc_dynamic_timing.patch:
//   - BYTE/WORD/DWORD -> uint8_t/uint16_t/uint32_t (MX's Core is all-stdint;
//     the win32api aliases are commented out in win32api/windows.h)
//   - ZeroMemory()    -> memset(..., 0, ...)      (no such macro in MX)

#include "crtc_timing.h"

// Register N is stored big-endian at bytes [2N] / [2N+1] (see CRTC_Write).
static uint16_t reg_word(const uint8_t *regs, int n)
{
    return (uint16_t)(((uint16_t)regs[n * 2] << 8) | regs[n * 2 + 1]);
}
...
void CrtcTiming_FromRegs(const uint8_t regs[48], int hrl, CrtcTiming *out)
{
    ...
    static const uint32_t osc_hz[3] = {
        CRTC_OSC_15K_HZ, CRTC_OSC_31K_HZ, CRTC_OSC_VGA_HZ
    };
    static const uint8_t osc_sel[16] = {
        0, 0, 0, 0,  1, 1, 1, 2,    // HRL=0
        0, 0, 0, 0,  1, 1, 1, 2     // HRL=1
    };
    static const uint8_t div_sel[16] = {
        8, 4, 8, 8,  6, 3, 2, 2,    // HRL=0
        8, 4, 8, 8,  8, 4, 2, 2     // HRL=1
    };

    uint8_t r20 = (uint8_t)(reg_word(regs, 20) & 0xff);
    int idx;
    int disp_cols;
    int disp_rasters;
...
void CrtcTiming_CyclesPerRaster(const CrtcTiming *t, uint32_t cpu_hz,
...
void CrtcTiming_CyclesPerField(const CrtcTiming *t, uint32_t cpu_hz,
    unsigned long long *num, unsigned long long *den)
{
...
int CrtcFieldClock_Next(CrtcFieldClock *clock, const CrtcTiming *timing,
    uint32_t cpu_hz, int current_vline_total, int *active_vline_total)
{
