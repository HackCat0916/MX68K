...
//
// MX68K port note (P641): the MPX68K original is written against the
// win32api shim types BYTE/WORD/DWORD. MX's Core was converted to <stdint.h>
// and those aliases are commented out in Core/px68k/win32api/windows.h, so
// this copy uses uint8_t/uint16_t/uint32_t. The widths are identical
// (unsigned char / unsigned short / unsigned int) -- no semantic change.
